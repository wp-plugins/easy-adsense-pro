...
This release is a fork of the "Pro" version found at http://buy.ads-ez.com/easy-adsense, with the forced ad-space donation and back-links ("Easy AdSense powered by Unreal") removed.
...
Easy AdSense is a commercial version of my extremely popular plugin of the same name originally hosted at WordPress.org. Due to its commercial nature, it is no longer hosted there. You can use the plugin in two modes:

* [Lite](http://buy.ads-ez.com/easy-adsense/easy-adsense.zip "Freely-distributed version of the Easy AdSense plugin"), where you share 5% of your ad slots with the author and sport a "powered-by" kind of backlinks under one of your ads.
* [Pro](https://buy.ads-ez.com/easy-adsense/ "Pro version of the Easy AdSense plugin"), which lets you edit the share fraction and other settings. The Pro version costs $4.95, and is appropriate if you expect to make more than $100 of ad revenue from your site.
...
Simplifying the code, removing unused functions etc. and minor bug fixes.
...
This plugin is distributed in two different versions. A freely-distributed version and a "Pro" version.

What you are getting for $4.95 in the Pro version is the ability to control ad-slot sharing and backlinks. Note that support is *not* included in the licensing fee. Each [support question](https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=HYZ5AWPYSC8VA "Ask a support question via PayPal") (for both the Lite and Pro versions) will be charged at $0.95. The same charges will apply for follow-up questions as well. In other words, support pricing is on a per-question basis, not a per-issue basis.

Further customization service is available upon request.

= Licensing terms, business model, etc. -- what do I need to know? =

Both the freely-distributed and the "Pro" versions of the plugin are released under the GPL license. It gives you specific rights regarding modification and using this plugin.

In the customized version of the plugin, one source code file (`custom.php`), which holds the default ads, customization and user/author information, is *not* under GPL, and you are not allowed to modify it. The customization service forms a transaction between the specific customer and the plugin author, and is not considered "distribution" under GPL.
...
Please copy *all* the files in the zip archive to your plugin directory. The zip archive `easy-adsense.?.??.zip` contains the directories `easy-adsense` and `easy-adsense/lang`. You need all the files in the `easy-adsense` directory. One of these files is the missing `defaults.php`. The `lang` directory, on the other hand, is optional and is meant for international users.
...
There is an ad-space sharing scheme implemented starting version 2.5 of Easy AdSense, if you would like to support its future development. It gives you an option to share a small fraction of your ad slots to show the author's ads. You can configure this fraction - the default is 0%. (Recommended is five percent if you are using the Lite version. This means my ad would appear roughly once in twenty of your ads). I have extra code to ensure that my ads don't appear more than once on the same page. You can control this ad space sharing by entering whatever fraction you like (in "Pro" version.).
...
* Chinese: `zh_CN`: [Akii Snow](http://www.akii.org "Akii Snow"), [ShenMeng](http://shenmeng.org "ShenMeng"), [Joojen](http://www.keege.com "Joojen"), [http://imxiaoku.co.cc](http://imxiaoku.co.cc "http://imxiaoku.co.cc"). `zh_TW`: [AKiRA](http://akr.tw/ "AKiRA").
